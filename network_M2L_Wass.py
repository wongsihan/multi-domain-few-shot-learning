...
class FourLayer_64F(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm1d, num_classes=5, neighbor_k=1, batch_size=1, shot_num=1,query_num = 15):
        ...
        #     nn.Conv1d(1, 64, kernel_size=10, stride=3, padding=0),
        #     norm_layer(64),
        #     nn.LeakyReLU(0.2, True),
        #     nn.MaxPool1d(2),  # 64*42*42
        #
        #     nn.Conv1d(64, 64, kernel_size=3, padding=0, bias=use_bias),
        #     norm_layer(64),
        #     nn.LeakyReLU(0.2, True),
        #     nn.MaxPool1d(2),  # 64*21*21
        #
        #     nn.Conv1d(64, 64, kernel_size=3,  padding=1, bias=use_bias),
        #     norm_layer(64),
...
class MML_Metric(nn.Module):
    ...
    def cal_covariance_matrix_Batch(self, feature):
        ...
        feature_mean = torch.mean(feature, 1, True)#(75,1,64)
        feature = feature - feature_mean
        cov_matrix = torch.matmul(feature.permute(0, 2, 1), feature)
        cov_matrix = torch.div(cov_matrix, n_local_descriptor - 1)#无偏估计
        cov_matrix = cov_matrix + 0.01 * torch.eye(cov_matrix.size(1)).cuda()#(75,64,64)
    ...
    def cal_MML_similarity(self, input1_batch, input2_batch):
        ...
        for i in range(1):
            input1 = input1_batch[i]#(4,75,441,64)->(75,441,64)
            input2 = input2_batch[i]#(4,5,441,64)->(5,441,64)
            ...
            was_dis = -self.wasserstein_distance_raw_Batch(query_mean, query_cov, s_mean, s_cov)#(75,5)
            ...
            innerproduct_matrix_l = torch.matmul(query_norm.unsqueeze(1),
                                                 support_norm_l.permute(0, 2, 1))#(75,5,441,441)
            innerproduct_matrix_p = torch.matmul(query_norm.permute(0, 2, 1).unsqueeze(1),
                                                 support_norm_p.permute(0, 2, 1))#(75,5,64,64)
            ...
            # parser = argparse.ArgumentParser(description="QS")
        ...
        #return Similarity_list, Q_S_List#[(75,5)()()()]
