...
from utlis.models3 import CNNEncoder1d, CNNEncoder2d, RelationNetwork1d, RelationNetwork2d,ProtoNet
from utlis.models3 import prototypical_loss as loss_fn
...
def main():
    ...
    for episode in range(EPISODE):
        ...
        # sample datas
        samples, sample_labels = sample_dataloader.__iter__().next()#5,1,1024
        batches, batch_labels = batch_dataloader.__iter__().next()#75,1,1024

        # calculate features
        sample_features_o = feature_encoder(Variable(samples).cuda(GPU).float())  # 5x64*83
        if MODELTYPE == '1d':
            sample_features = sample_features_o.view(CLASS_NUM*SAMPLE_NUM_PER_CLASS, FEATURE_DIM)
        else:
        ...
        batch_features = feature_encoder(Variable(batches.float()).cuda(GPU)).squeeze()  # 75,64,1

        # training
        model_output = [sample_features,batch_features]
        model_output = torch.cat(model_output,dim=0)
        y = [sample_labels,batch_labels]
        y = torch.cat(y,dim=0)

        loss, acc = loss_fn(model_output, target=y,
                            n_support=SAMPLE_NUM_PER_CLASS)
        ...
        logging.info("episode:" + str(episode + 1) + "   loss: " + str(loss.item()) + "  train_acc: " + str(acc.item()))
        ...
        if (episode + 1) % testgap == 0:
            ...
            for i in range(TEST_EPISODE):
                ...
                if MODELTYPE == '1d':
                    sample_features = sample_features.view(CLASS_NUM*SAMPLE_NUM_PER_CLASS, FEATURE_DIM)
                else:
                    sample_features = sample_features.view(CLASS_NUM, SAMPLE_NUM_PER_CLASS, FEATURE_DIM, 5, 5)
                #sample_features = torch.mean(sample_features, 1).squeeze(1)
                test_features = feature_encoder(Variable(test_images).cuda(GPU).float()).squeeze()  # 20x64
                ...
                model_output = [sample_features, test_features]
                model_output = torch.cat(model_output, dim=0)
                y = [sample_labels, test_labels]
                y = torch.cat(y, dim=0)
            ...
            test_accuracy = np.mean(val_acc[-TEST_EPISODE:])
    ...
    logging.info("last accuracy :" + str(test_accuracy))
    #logging.info("final accuracy :" + str(finalsum/4))
    plt.figure()
    plt.suptitle(path1)
    plt.subplot(2, 1, 1)
