...
def main():
    ...
    for episode in range(EPISODE):
        ...
        task2 = tg.puTask(metatest_character_folders, CLASS_NUM, SAMPLE_NUM_PER_CLASS, BATCH_NUM_PER_CLASS)
        ...
        sample_dataloader2 = tg.get_data_loader(task2, num_per_class=SAMPLE_NUM_PER_CLASS, split="train", shuffle=False,
                                               dt=DATATYPE, mt=MODELTYPE)
        ...
        # sample datas
        samples, sample_labels = sample_dataloader.__iter__().next()#(5,1,1024)(5,)
        samples2, sample_labels2 = sample_dataloader2.__iter__().next()
        batches, batch_labels = batch_dataloader.__iter__().next()#(75,1,1024)(75,)

        # calculate features
        sample_features_o = feature_encoder(Variable(samples).cuda(GPU).float())  # (5,64,25)
        if MODELTYPE == '1d':
