...
from network_M2L_Wass import define_FewShotNet,accuracy
...
def main():
    ...
    for episode in range(EPISODE):
        ...
        output = feature_encoder(Variable(batches).cuda(GPU).float(),
                                 Variable(samples).cuda(GPU).float())
        ...
        if (episode + 1) % testgap == 0:
            ...
            for i in range(TEST_EPISODE):
                ...
                output = feature_encoder(Variable(test_images).cuda(GPU).float(),
                                         Variable(sample_images).cuda(GPU).float())
