...
def mytest(metatrain_character_folders,metatest_character_folders, feature_encoder):
    ...
    for episode in range(TEST_EPISODE):
        ...
        task2 = tg.finetuneTask(metatrain_character_folders, SAMPLE_NUM_PER_CLASS)
        support_dataloader = tg.get_test_loader(task2, SAMPLE_NUM_PER_CLASS, 'support', CLASS_NUM_test)
        test_dataloader = tg.get_test_loader(task, batchsize, 'test', CLASS_NUM_test,snr = SNR)
        accuracys = []
...
def main():
    ...
    acc = mytest(metatrain_character_folders,metatest_character_folders, feature_encoder)
