...
from utlis.models2 import CNNEncoder1d, CNNEncoder2d, RelationNetwork1d, RelationNetwork2d,ImgtoClass_Metric
...
def accuracy(output, target, topk=(1,)):
	...
	with torch.no_grad():
		maxk = max(topk)#3
		batch_size = target.size(0)#5

		_, pred = output.topk(maxk, 1, True, True)#(5,3) (5,5)
		pred = pred.t()
...
def main():
    ...
    if MODELTYPE == '1d':
        ...
        siganl2class = ImgtoClass_Metric(3)
    elif MODELTYPE == '2d':
    ...
    for episode in range(EPISODE):
        ...
        # calculate features
        sample_features_o = feature_encoder(Variable(samples).cuda(GPU).float()) # 5x64*25
        if MODELTYPE == '1d':
            sample_features = sample_features_o.view(CLASS_NUM, SAMPLE_NUM_PER_CLASS, FEATURE_DIM, 25)
        else:
            sample_features = sample_features_o.view(CLASS_NUM, SAMPLE_NUM_PER_CLASS, FEATURE_DIM, 5, 5)
        sample_features = torch.mean(sample_features, 1).squeeze(1)# 5,64,25
        #sample_features = sample_features.squeeze(1)
        batch_features = feature_encoder(Variable(batches.float()).cuda(GPU))  # 75,64,25
        batch_features = batch_features.view(CLASS_NUM*BATCH_NUM_PER_CLASS,  FEATURE_DIM, 5,5)#75,64,5,5
        ...
        S = []
        if MODELTYPE == '1d':
            b, c, h = sample_features.size()
            for i in range(b):
                support_set_sam = sample_features[i]
                S.append(support_set_sam)
            #sample_features_ext = sample_features.unsqueeze(0).repeat(BATCH_NUM_PER_CLASS * CLASS_NUM, 1, 1, 1)#75,5,64,25
            # batch_features_ext = batch_features.unsqueeze(0).repeat(CLASS_NUM, 1, 1, 1)
            # batch_features_ext = torch.transpose(batch_features_ext, 0, 1)#75,5,64,25
            # relation_pairs = torch.cat((sample_features_ext, batch_features_ext), 2).view(-1, FEATURE_DIM * 2, 5 * 5)#375,128,25
        else:
        ...
        #relations = relation_network(relation_pairs).view(-1, CLASS_NUM)#75,5
        relations = siganl2class(batch_features,S)
        prec1, _ = accuracy(relations, batch_labels, topk=(1, 3))
        ...
        loss = crossentropy(relations+1e-10,batch_labels)
        # training
        ...
        logging.info("episode:" + str(episode + 1) + "   loss: " + str(loss.item())+"   acc: " + str(prec1.item()))
        ...
        if (episode + 1) % testgap == 0:
            ...
            for i in range(TEST_EPISODE):
                ...
                test_features = feature_encoder(Variable(test_images).cuda(GPU).float()) # 5,64,25
                test_features = test_features.view(CLASS_NUM*BATCH_NUM_PER_CLASS,  FEATURE_DIM, 5,5)
                # calculate relations
                # each batch sample link to every samples to calculate relations
                S2 = []
                if MODELTYPE == '1d':
                ...
                relations = siganl2class(test_features, S2)#75,5
                ...
                prec1, _ = accuracy(relations, test_labels, topk=(1, 3))
                total_prec +=  prec1 / 100 * CLASS_NUM * SAMPLE_NUM_PER_CLASS
            test_accuracy = total_prec / 1.0 / CLASS_NUM / SAMPLE_NUM_PER_CLASS / TEST_EPISODE
            #test_accuracy = total_prec / TEST_EPISODE
